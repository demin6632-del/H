...
# Улучшаем разрешение V4-атласа только средствами стандартного Ubuntu runner.
# Старый V4 содержит 256x384 (64x64 на клетку); новый — 1024x1536 (256x256 на клетку).
for p in [Path('NEW_DARK_RPG/art-atlas-v4.js'), Path('android/app/src/main/assets/art-atlas-v4.js')]:
    s=p.read_text(encoding='utf-8')
    ...
    with tempfile.TemporaryDirectory() as td:
        src=Path(td)/'src.webp';dst=Path(td)/'dst.webp';src.write_bytes(raw)
        subprocess.run([
            'ffmpeg','-hide_banner','-loglevel','error','-y','-i',str(src),
            '-vf','scale=1024:1536:flags=lanczos,unsharp=5:5:0.8:5:5:0',
            '-c:v','libwebp','-quality','92',str(dst)
        ],check=True)
