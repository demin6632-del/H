...
def patch(path):
    ...
    if 'const equippable=x.type===' not in s:
        if INVENTORY_OLD in s:
            s=s.replace(INVENTORY_OLD, INVENTORY_NEW, 1)
        else:
            raise SystemExit(f'Inventory pattern not found in {path}')
    if 'const setSlot=(id,icon,name)=>' not in s:
        if not EQUIPMENT_RE.search(s):
            raise SystemExit(f'Equipment function not found in {path}')
