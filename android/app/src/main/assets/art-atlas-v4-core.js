...
function generatedKeyV4(key){const map={start:'abyss_gate',ordinary:'void_bridge',treasure:'ancient_gate',shop:'obsidian_hall',forge:'iron_ruins',tavern:'black_keep',sanctuary:'abyss_core',cursed:'bone_catacomb',library:'cursed_library',altar:'blood_rift',trap:'red_chasm',labyrinth:'shadow_forest',portal:'abyss_core',bossroom:'rift_lord',eliteroom:'archon_chamber',event:'endless_depth'};return map[key]||'void_bridge';}
function generatedCreatureV4(key){return key==='hero'?'hero':key;}
function renderV4Tile(el,key,w,h){if(!el)return;const name=generatedKeyV4(key);el.style.setProperty('background-image','linear-gradient(rgba(5,0,4,.18),rgba(0,0,0,.42)),url("assets/generated/'+name+'.png")','important');el.style.setProperty('background-size','cover','important');el.style.setProperty('background-position','center center','important');el.style.setProperty('background-repeat','no-repeat','important');el.style.setProperty('image-rendering','auto','important');el.dataset.v4QualityKey=name;return true;}
...
function installBattleV4(){const old=window.updateBattle;if(typeof old!=='function'||old.__artV4)return false;function wrapped(){old.apply(this,arguments);const b=document.getElementById('enemyArt');if(!b)return;let k='shadow';if(typeof enemy!=='undefined'&&enemy){if(enemy.isBoss)k='boss';else if(enemy.isElite)k='elite';else k=({'Теневой зверь':'shadow','Заражённый охотник':'hunter','Мутант пустоши':'mutant','Пожиратель костей':'bones'})[enemy.name]||'shadow';}b.innerHTML='<img src="assets/generated/'+generatedCreatureV4(k)+'.png" alt="" style="display:block;width:min(86vw,360px);height:min(86vw,300px);object-fit:cover;object-position:center;margin:auto;border:1px solid rgba(45,125,170,.65);box-shadow:0 0 22px rgba(0,70,110,.28)">';}wrapped.__artV4=true;wrapped.__battleArtRuntimeFix=true;window.updateBattle=wrapped;return true;}
